% Evolutionary invasion analysis on resource-explicit - NonSuperinfection 
...
for i = 1:length(alphaS_set)
    for j = 1:length(theta_set)
       ...
       if feasible_strategy_flag % if resident strategy can invade
           ...
           R_mut = calculateR0_NoSuperinfection(p, [x(end,1), x(end,2)], [p.pb_m, p.gamma_m]);
...
fid = fopen('data_robustness_evo_robustness_resource_explicit_NoSuperinfection.txt','wt');
...
figure(2);
subplot(1,3,1);
maph = [ 0.3010 0.7450 0.9330 % light blue
         0 0.4470 0.7410      % faded blue
         0.8 0.8 0.8 % gray
         0 0 1 % blue    
         1 0 0]; % gray
...
colormap(maph);
plot(S_set, R_c, 'k', 'LineWidth',3); hold on;

p.J = event_lytic_robust(1);
p.alphaS = event_lytic_robust(2);
R_star = p.Rmod*p.ds/((1 - p.alphaS)*p.b0 - p.ds);
S_star = (p.J - p.rho*R_star)/(p.e*p.ds);
plot(S_star, R_star,'pk','MarkerSize',20); hold on;

p.J = event_lytic_non_robust(1);
p.alphaS = event_lytic_non_robust(2);
R_star = p.Rmod*p.ds/((1 - p.alphaS)*p.b0 - p.ds);
S_star = (p.J - p.rho*R_star)/(p.e*p.ds);
plot(S_star, R_star,'dk','MarkerSize',20); hold on;

set(gca,'XScale','log'); axis square;
set(gca,'FontSize',20);
ax = gca; ax.XAxis.LineWidth = 2; ax.YAxis.LineWidth = 2;
xticks([1e5 1e6 1e7 1e8]); 
xlabel('Susceptible population, S^{*}', 'FontName', 'Times New Roman','FontSize',25); 
ylabel('Resources, R^{*}','FontName', 'Times New Roman','FontSize',25);


p.pb = 0; p.gamma = 1;
p.pbm = 1; p.gamma_m = 1e-2;
...
R_star = p.Rmod*p.ds/((1 - p.alphaS)*p.b0 - p.ds);
S_star = (p.J - p.rho*R_star)/(p.e*p.ds);
t0 = 0; tf = 6e2; tol = 1e-5; 
options = odeset('RelTol',tol,'AbsTol',tol);
IC = [R_star,S_star,0,0,0,1e2,0,0,0,0]; % initial condition
[t,x] = ode45(@ode_resource_explicit,[t0,tf],IC,options,p);
t0 = 0; tf1 = 3e2;
IC = [x(end,1),x(end,2),x(end,3),x(end,4),x(end,5),x(end,6),0,0,0,1e2];
[t1,x1] = ode45(@ode_resource_explicit,[t0,tf1],IC,options,p);
% semilogy([t;t1 + tf], [x(:,1);x1(:,1)], 'k', 'LineWidth',2); hold on; 
semilogy([t;t1 + tf], [x(:,2);x1(:,2)], 'b', 'LineWidth',2); hold on; 
semilogy([t;t1 + tf], [x(:,3);x1(:,3)], 'g', 'LineWidth',2); hold on; 
semilogy([t;t1 + tf], [x(:,4);x1(:,4)], 'm', 'LineWidth',2); hold on; 
semilogy([t;t1 + tf], [x(:,5);x1(:,5)], 'y', 'LineWidth',2); hold on; 
semilogy([t;t1 + tf], [x(:,6);x1(:,6)], 'r', 'LineWidth',2); hold on; 
semilogy([t;t1 + tf], [x(:,7);x1(:,7)], '--g', 'LineWidth',2); hold on; 
semilogy([t;t1 + tf], [x(:,8);x1(:,8)], '--m', 'LineWidth',2); hold on; 
semilogy([t;t1 + tf], [x(:,9);x1(:,9)], '--y', 'LineWidth',2); hold on;
semilogy([t;t1 + tf], [x(:,10);x1(:,10)], '--r', 'LineWidth',2); 
legend({'$S$','$E$','$L$','$I$','$V$','$E_{m}$','$L_{m}$','$I_{m}$','$V_{m}$'},...
    'Interpreter','latex','FontSize',15); legend boxoff;       
ylim([1 1e10]); xlim([0 tf + tf1]); axis square;
set(gca,'FontSize',20);
ax = gca; ax.XAxis.LineWidth = 2; ax.YAxis.LineWidth = 2;
xlabel('Time', 'FontName', 'Times New Roman','FontSize',25); 
ylabel('Density', 'FontName', 'Times New Roman','FontSize',25); 

figure(3);
subplot(1,2,1);
plot([t;t1 + tf], [x(:,1);x1(:,1)], 'k', 'LineWidth',2); axis square;
xlabel('Time', 'FontName', 'Times New Roman','FontSize',25); 
ylabel('Resources', 'FontName', 'Times New Roman','FontSize',25); 

% Free viral particles can be reproduced via lysogens
function dxdt = ode_resource_explicit_KillLysogens(t,x,p)
% population states
...
dedt = p.phi*V*(S + L_tot) - p.alpha*E - p.de*E;
dldt = bl*L + p.pb*p.alpha*E - p.gamma*L - p.dl*L - p.phi*L*V_tot;
didt = (1 - p.pb)*p.alpha*E - p.eta*I + p.gamma*L - p.di*I;
dvdt = p.beta*p.eta*I - p.phi*N*V - p.m*V;
% Mutant system
demdt = p.phi*Vm*(S + L_tot) - p.alpha*Em - p.de*Em;

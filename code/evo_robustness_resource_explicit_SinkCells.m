...
for i = 1:length(alphaS_set)
    for j = 1:length(theta_set)
       ...
       if feasible_strategy_flag % if resident strategy can invade
           % resident dynamics
           [t,x] = ode45(@ode_resource_explicit_SinkCells,[t0,tf],IC,options,p);
           % compute invasion fitness of mutant strategy
           R_mut = calculateR0_SinkCells(p, x(end,1:5), [p.pb_m, p.gamma_m]);
...
fid = fopen('data_robustness_evo_robustness_resource_explicit_SinkCells.txt','wt');

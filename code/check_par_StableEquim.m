% Verify the parameter space (stable equilibrium)
% Plot the coarse grid of parameter space to check.
%% Model parameters
...
% coarse parameter space
theta_set = [0.01, 0.05, 0.1, 0.5, 1, 5, 10, 15];
alphaS_set = [-1, -0.8, -0.5, -0.2, 0, 0.2, 0.4];
...
for i = 1:length(alphaS_set)
    for j = 1:length(theta_set)
       ...
       if feasible_strategy_flag % if resident strategy can invade
           ...
           semilogy(t,x + 1e-2);
           figure(1);
           semilogx(S_star, R_star, 'or'); hold on;
       else % Not feasible
           figure(1);
           semilogx(S_star, R_star, '+k'); hold on;       
       end
